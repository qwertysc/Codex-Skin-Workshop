...
- **独立主题文件**：内置模板位于 `src/themes/*.json`，用户主题可导入、导出和分享。
- **完整可视化参数**：背景位置与缩放、面板透明度与模糊、圆角、内容宽度、十组颜色、主题文案和装饰元素。
...
当前后端采用托管 Codex 预览进程和仅限回环地址的 DevTools 连接，不直接修改应用文件。启动时会等待真正可注入的 renderer target；用户退出 Codex 后再次点击“应用到 Codex”，会清理失效状态并重新启动。

## 主题分享格式

主题文件是经过 Rust 后端严格校验的纯 JSON 数据，不允许携带 CSS、JavaScript、HTML、Shell 或 PowerShell。导出时会移除本机背景图片路径；接收者导入后可以重新选择自己的背景图片。
