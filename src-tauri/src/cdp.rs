...
pub fn evaluate(raw: &str, port: u16, id: &str, expression: &str) -> Result<(), AppError> {
    ...
    socket.send(Message::Text(json!({"id":1,"method":"Runtime.evaluate","params":{"expression":expression,"awaitPromise":true,"returnByValue":true}}).to_string())).map_err(|e| AppError::Cdp(e.to_string()))?;
    loop {
        match socket.read() {
...
mod tests {
    ...
    #[test] fn payload_contains_only_encoded_user_values() {
        ...
        assert!(payload.contains("codex-skin-workshop-style")); assert!(!payload.contains("#abcdef")); assert!(!payload.contains("--csw-accent"));
    }
    #[test] fn restore_is_fixed_and_scoped() { assert!(RESTORE_PAYLOAD.contains("codex-skin-workshop-style")); assert!(RESTORE_PAYLOAD.contains(BACKDROP_NODE_ID)); assert!(!RESTORE_PAYLOAD.contains("querySelectorAll")); }
}
